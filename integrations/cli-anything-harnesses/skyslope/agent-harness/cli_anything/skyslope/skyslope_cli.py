...
Usage:
    cli-anything-skyslope gate status                       # offline: is the ECC gate open?
    cli-anything-skyslope verbs                             # offline: reads that exist, writes deliberately absent
    cli-anything-skyslope paths show|validate               # offline: the (unverified) path map
    cli-anything-skyslope --json recipe transactions        # gated, read-only
    cli-anything-skyslope --json recipe document-index --id TX-1
    cli-anything-skyslope discover --url https://app.skyslope.com/transactions
    cli-anything-skyslope                                   # REPL
...
from cli_anything.skyslope import __version__
from cli_anything.skyslope.core import discover as discover_mod
from cli_anything.skyslope.core import paths as paths_mod
from cli_anything.skyslope.core import policy
from cli_anything.skyslope.core import recipes as recipes_mod
...
def cli(ctx, use_json, paths_file):
    """SkySlope CLI — read-only, ECC-gated.
...
def _make_recipe_command(name: str, spec: dict) -> click.Command:
    ...
    if spec["needs_id"]:
        params.append(click.Option(["--id", "id_"], required=True,
                                   help="Transaction id (letters, digits, - and _ only)"))
...
@gated
def page_open(url):
    """Open an allow-listed SkySlope URL."""
...
def repl():
    ...
    from cli_anything.skyslope.utils.repl_skin import ReplSkin
