"""Unit tests for cli-anything-skyslope — offline, mocked, synthetic.
...
from cli_anything.skyslope.core import discover, paths as paths_mod, policy, recipes, target
from cli_anything.skyslope.utils import security
from cli_anything.skyslope.utils import skyslope_backend as backend
from cli_anything.skyslope import skyslope_cli

GATE = policy.ENV_VAR
PKG_DIR = Path(skyslope_cli.__file__).resolve().parent
...
class FakeBackend:
    ...
    def __init__(self, tree=None, grep_results=None, cat_results=None,
        ...
        self.tree = tree or {"/main": ["TX-SAMPLE-1 100 Example Way (fixture)",
                                       "TX-SAMPLE-2 200 Example Way (fixture)"]}
...
def _reset_cli_state():
    skyslope_cli._availability_cached = None
    skyslope_cli._json_output = False
    skyslope_cli._paths_override = None
    yield
    skyslope_cli._availability_cached = None


def _invoke(args, env):
    return CliRunner().invoke(skyslope_cli.cli, args, env=env)
...
class TestCLIGate:
    def test_recipe_refused_without_date_exit_3(self):
        with patch.object(backend, "is_available") as avail:
            r = _invoke(["recipe", "transactions"], env=NO_GATE)
    ...
    def test_recipe_refused_json_shape(self):
        r = _invoke(["--json", "recipe", "document-index", "--id", "TX-1"], env=NO_GATE)
    ...
    @pytest.mark.parametrize("args", [
        ...
        ["fs", "pwd"], ["page", "open", "https://app.skyslope.com/"], ["page", "info"],
        ["page", "reload"], ["page", "back"], ["page", "forward"],
        ["recipe", "checklist-status", "--id", "TX-1"],
        ["recipe", "transaction-detail", "--id", "TX-1"],
    ])
    ...
    def test_future_date_refused_at_cli(self):
        r = _invoke(["recipe", "transactions"], env={GATE: "2999-01-01"})
    ...
    def test_gate_open_but_domshell_missing_is_dependency_error(self):
        with patch.object(backend, "is_available", return_value=(False, "npx not found (fixture)")):
            r = _invoke(["--json", "recipe", "transactions"], env=OPEN_GATE)
    ...
    def test_gate_open_recipe_runs_through_mocked_seam(self):
        ...
        assert payload["rows"][0]["name"].startswith("TX-SAMPLE-1")
    ...
    def test_gate_status_json(self):
        ...
        assert d["connState"] == "disabled-by-policy" and d["target"] == "SkySlope"
...
class TestNoWriteSurface:
    ...
    def test_no_act_group(self):
        assert "act" not in skyslope_cli.cli.commands
    ...
    def test_no_group_carries_a_write_subcommand(self):
        for gname, group in skyslope_cli.cli.commands.items():
    ...
    def test_verbs_command_documents_the_absent_writes(self):
        ...
        assert "esign send" in disabled and "document download" in disabled
...
class TestRecipes:
    def test_transactions_rows_and_calls(self, path_map):
        fake = FakeBackend()
        out = recipes.run("transactions", path_map, backend_mod=fake)
        assert out["count"] == 2 and out["verified"] is False and out["target"] == "SkySlope"
        assert out["url"] == "https://app.skyslope.com/transactions"
    ...
    def test_document_index_lists_names_and_never_downloads(self, path_map):
        fake = FakeBackend(tree={"/main": ["Fixture Disclosure.pdf", "Fixture Addendum.pdf"]})
        out = recipes.run("document-index", path_map, id="TX-1", backend_mod=fake)
        assert out["url"].endswith("/transactions/TX-1/documents")
        assert [r["name"] for r in out["rows"]] == ["Fixture Disclosure.pdf", "Fixture Addendum.pdf"]
        assert {c[0] for c in fake.calls} <= {"new_session", "open_url", "grep", "ls"}
        assert "download" not in out["note"].lower() or "no download verb" in out["note"]

    def test_transaction_detail_reads_text(self, path_map):
        fake = FakeBackend(cat_results={"/main": {"output": "Fixture summary text"}})
        out = recipes.run("transaction-detail", path_map, id="TX-9", backend_mod=fake)
        assert out["text"] == "Fixture summary text"

    def test_checklist_status_reports_matches(self, path_map):
        fake = FakeBackend(grep_results={"Incomplete": ["/main/item[2]"]})
        out = recipes.run("checklist-status", path_map, id="TX-9", backend_mod=fake)
        assert out["matches"] == ["/main/item[2]"]
    ...
    def test_id_required_and_sanitised(self, path_map):
        with pytest.raises(paths_mod.PathMapError):
            recipes.run("document-index", path_map, backend_mod=FakeBackend())
        for bad in ("../x", "a/b", "x?y=1", "a b", "x" * 65):
            with pytest.raises(paths_mod.PathMapError):
                recipes.run("document-index", path_map, id=bad, backend_mod=FakeBackend())

    def test_unknown_recipe(self, path_map):
        with pytest.raises(ValueError):
            recipes.run("esign-send", path_map, backend_mod=FakeBackend())

    def test_recipe_names_match_spec(self):
        assert set(target.RECIPES) == {"transactions", "transaction-detail", "document-index", "checklist-status"}


# ── URL allow-list (utils/security.py) and the seam's refusal ────
class TestSecurity:
    @pytest.mark.parametrize("url", ["https://app.skyslope.com/", "https://app.skyslope.com/transactions/TX-1"])
    def test_allowed(self, url):
        assert security.validate_target_url(url) == (True, "")

    @pytest.mark.parametrize("url", [
        "http://app.skyslope.com/", "https://example.com/", "https://app.skyslope.com.evil.example/",
        "https://user:pw@app.skyslope.com/", "javascript:alert(1)", "file:///etc/hosts",
...
class TestPathMap:
    ...
    def test_validation_catches_bad_entries(self, path_map):
        ...
        data["recipes"]["transactions"]["url"] = "https://example.com/x"
        data["recipes"]["document-index"]["url"] = "https://app.skyslope.com/docs"   # no {id}
        data["recipes"]["checklist-status"]["list_path"] = "relative"
        del data["recipes"]["transaction-detail"]
        ...
        assert any("transaction-detail" in e and "missing" in e for e in errors)
...
class TestSeamBinding:
    ...
    def test_open_url_allowed_host_reaches_browser_page(self):
        ...
        with patch("cli_anything.browser.core.page.backend.open_url",
                   return_value={"output": "ok", "url": "https://app.skyslope.com/"}) as m:
            out = backend.open_url(sess, "https://app.skyslope.com/")
        m.assert_called_once()
        assert "error" not in out and sess.current_url == "https://app.skyslope.com/"
...
def _resolve_cli(name: str) -> list[str]:
    ...
    print(f"[_resolve_cli] Falling back to python -m cli_anything.skyslope")
    return [sys.executable, "-m", "cli_anything.skyslope"]
...
class TestInstalledCommand:
    ...
    def test_recipe_refused_exit_3(self):
        r = self._run(["--json", "recipe", "transactions"])
