"""Path-map loading for the public-feeds harness (market pages, lender rate
pages, builder incentive pages — see target.py for the per-recipe policy group).
...
1. ``$CLI_ANYTHING_PUBLICFEEDS_PATHS`` — an explicit file path.
2. ``~/.config/cli-anything/publicfeeds-paths.json`` — Steven's edited copy.
3. The packaged default next to this package (``cli_anything/publicfeeds/paths.json``).
...
from cli_anything.publicfeeds import SITE

ENV_VAR = "CLI_ANYTHING_PUBLICFEEDS_PATHS"
