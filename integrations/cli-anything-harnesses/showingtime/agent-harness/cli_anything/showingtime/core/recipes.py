...
A *recipe* is a named, read-only path map into one ShowingTime page::
...
from cli_anything.showingtime import SITE, __version__
from cli_anything.showingtime.utils.security import require_site_url
...
def _resolve_url(cfg: dict, pm: dict, url: Optional[str]) -> str:
    ...
    if not target:
        raise RecipeError(
            "this recipe needs --url (a page on ShowingTime to read)", kind="usage_error",
        )
