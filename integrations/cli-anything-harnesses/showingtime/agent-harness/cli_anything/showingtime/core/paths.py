"""Path-map loading for the ShowingTime harness.
...
1. ``$CLI_ANYTHING_SHOWINGTIME_PATHS`` — an explicit file path.
2. ``~/.config/cli-anything/showingtime-paths.json`` — Steven's edited copy.
3. The packaged default next to this package (``cli_anything/showingtime/paths.json``).
...
from cli_anything.showingtime import SITE

ENV_VAR = "CLI_ANYTHING_SHOWINGTIME_PATHS"
