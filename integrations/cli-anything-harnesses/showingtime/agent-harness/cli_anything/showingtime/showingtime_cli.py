...
Usage:
    cli-anything-showingtime --json recipes                       # what can be read
    cli-anything-showingtime --json recipe <name> [--url U]       # one recipe → JSON
    cli-anything-showingtime --json recipe <name> --discover      # dump the live tree to fix paths.json
    cli-anything-showingtime --json fs ls /                       # raw tree, same as the browser harness
...
from cli_anything.showingtime import CLI_NAME, SITE, SITE_TITLE, __version__
from cli_anything.showingtime.core import paths as paths_mod
from cli_anything.showingtime.core import recipes as recipes_mod
from cli_anything.showingtime.core.auth import AuthError
from cli_anything.showingtime.core.recipes import RecipeError
from cli_anything.showingtime.core.tree import LiveTree, TreeError
from cli_anything.showingtime.utils.security import URLRejected, require_site_url
...
EXIT_URL_REJECTED = 4   # URL failed the ShowingTime allow-list
...
def cli(ctx, use_json):
    """ShowingTime read-only CLI — named page recipes over the DOMShell browser harness.
...
def paths_init(force):
    """Copy the packaged default map to ~/.config/cli-anything/showingtime-paths.json for editing."""
...
@cli.group()
def page():
    """Page navigation, read-only. `open` accepts only https ShowingTime URLs."""
...
@handle_error
def page_open(url):
    """Open a ShowingTime URL (allow-listed hosts, https only)."""
...
def repl():
    ...
    from cli_anything.showingtime.utils.repl_skin import ReplSkin
