...
1. `cli-anything-showingtime --help` exits 0, and `cli-anything-showingtime --help | grep -qw act` finds nothing (exit 1). A **word** match: a substring match false-trips on `my-listing-activity`, `redact`, `contact`, `interactive`, `exact` — **done, mechanical, in the test suite.**
