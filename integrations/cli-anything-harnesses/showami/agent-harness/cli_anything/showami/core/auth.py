...
def check_auth(source: TreeSource, auth_cfg: dict, landed_url: str, required: bool) -> str:
    ...
    for frag in auth_cfg.get("login_url_fragments", []) or []:
        if frag and frag.lower() in url_l:
            raise AuthError(
                f"Showami redirected to a sign-in page ({landed_url}). "
                f"Sign in to Showami in Chrome, then re-run.",
                state="logged_out",
    ...
    if out_marker:
        raise AuthError(
            f"Showami page shows '{out_marker}' and no signed-in marker — you are signed out. "
            f"Sign in to Showami in Chrome, then re-run.",
            state="logged_out",
            hint="Sign in by hand in the Chrome profile DOMShell drives; this harness never types credentials.",
        )
    ...
    raise AuthError(
        "Could not confirm a signed-in Showami session: none of auth.logged_in_markers "
