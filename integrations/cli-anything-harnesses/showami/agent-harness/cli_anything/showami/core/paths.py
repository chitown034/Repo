"""Path-map loading for the Showami harness.
...
1. ``$CLI_ANYTHING_SHOWAMI_PATHS`` — an explicit file path.
2. ``~/.config/cli-anything/showami-paths.json`` — Steven's edited copy.
3. The packaged default next to this package (``cli_anything/showami/paths.json``).
...
from cli_anything.showami import SITE

ENV_VAR = "CLI_ANYTHING_SHOWAMI_PATHS"
