...
A *recipe* is a named, read-only path map into one Showami page::
...
from cli_anything.showami import SITE, __version__
from cli_anything.showami.utils.security import require_site_url
...
def _resolve_url(cfg: dict, pm: dict, url: Optional[str]) -> str:
    ...
    if not target:
        raise RecipeError(
            "this recipe needs --url (a page on Showami to read)", kind="usage_error",
        )
