#!/usr/bin/env python3
"""cli-anything-showami — read-only Showami recipes over the DOMShell browser harness.

Usage:
    cli-anything-showami --json recipes                       # what can be read
    cli-anything-showami --json recipe <name> [--url U]       # one recipe → JSON
    cli-anything-showami --json recipe <name> --discover      # dump the live tree to fix paths.json
    cli-anything-showami --json fs ls /                       # raw tree, same as the browser harness
...
from cli_anything.showami import CLI_NAME, SITE, SITE_TITLE, __version__
from cli_anything.showami.core import paths as paths_mod
from cli_anything.showami.core import recipes as recipes_mod
from cli_anything.showami.core.auth import AuthError
from cli_anything.showami.core.recipes import RecipeError
from cli_anything.showami.core.tree import LiveTree, TreeError
from cli_anything.showami.utils.security import URLRejected, require_site_url
...
EXIT_URL_REJECTED = 4   # URL failed the Showami allow-list
...
def cli(ctx, use_json):
    """Showami read-only CLI — named page recipes over the DOMShell browser harness.
...
def paths_init(force):
    """Copy the packaged default map to ~/.config/cli-anything/showami-paths.json for editing."""
...
@cli.group()
def page():
    """Page navigation, read-only. `open` accepts only https Showami URLs."""
...
@handle_error
def page_open(url):
    """Open a Showami URL (allow-listed hosts, https only)."""
...
def repl():
    ...
    from cli_anything.showami.utils.repl_skin import ReplSkin
