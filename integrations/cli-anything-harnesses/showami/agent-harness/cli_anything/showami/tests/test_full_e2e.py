"""LIVE tests for cli-anything-showami — never run in the build sandbox.

These require, on the Mac: Chrome running with the DOMShell extension, Node/npx,
``DOMSHELL_TOKEN`` exported, and Steven signed in to Showami in that
...
explicit opt-in so an unattended ``pytest`` can never touch a live portal:
    ...
    CLI_ANYTHING_SHOWAMI_LIVE=1 python -m pytest cli_anything/showami/tests/test_full_e2e.py -v -s
...
from cli_anything.showami import CLI_NAME
from cli_anything.showami.core import paths as paths_mod

LIVE = os.environ.get("CLI_ANYTHING_SHOWAMI_LIVE", "").strip() == "1"

pytestmark = pytest.mark.skipif(
    not LIVE,
    reason="live Showami tests are opt-in: set CLI_ANYTHING_SHOWAMI_LIVE=1 on the Mac with DOMShell running and a signed-in session",
)
