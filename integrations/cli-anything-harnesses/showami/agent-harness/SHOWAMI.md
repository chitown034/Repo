# Showami Harness: read-only recipes over DOMShell
...
Showami has no usable API for an individual agent (see `.claude/skills/cli-anything-connectors/SKILL.md`). The only
...
```
cli-anything-showami recipe <name> --json
        │
        ▼
...
- has **no `act` group** — not disabled, absent. `showami_cli.py` defines `fs`, `page`, `recipe`, `recipes`,
...
- allow-lists `page open` and every recipe URL to https on `showami.com` and every subdomain.
...
profile DOMShell drives. Sign in by hand in Chrome. Nothing in this package posts, accepts, cancels or messages — every one of those is money or a third party at a stranger's door.
...
**`my-requests`** — root `['/main']`, rows `['listitem', 'article', 'row', 'group']`, depth 1:

| Field | How it is found |
|---|---|
| `request_id` | regex `#\s*(\d{3,})` group 1 |
| `address` | child named `link` or `heading` |
| `when` | regex `((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.? \d{1,2}(?:, \d{4})?(?:,?\s*(?:at\s*)?\d{1,2}:\d{2}\s*[AaPp][Mm])?)` |
| `status` | regex `(Open|Posted|Pending|Accepted|Assigned|Confirmed|Completed|Cancelled|Canceled|Expired|Unfilled)` |
| `showing_assistant` | regex `(?:Showing Assistant|Assistant|Accepted by)\s*:?\s*([^\n]+)` group 1 |
| `price` | regex `\$\s?\d+(?:\.\d{2})?` |
| `showing_type` | regex `(Buyer Showing|Listing Showing|Inspection|Appraisal|Walk-?Through|Open House|Photo Shoot)` |

**`request-status`** — root `['/main']`, rows `['listitem', 'article', 'row', 'group']`, depth 1:
...
**`assistant-feedback`** — root `['/main']`, rows `['listitem', 'article', 'row', 'group']`, depth 1:

| Field | How it is found |
|---|---|
| `request_id` | regex `#\s*(\d{3,})` group 1 |
| `address` | child named `link` or `heading` |
| `showing_assistant` | regex `(?:Showing Assistant|Assistant|From)\s*:?\s*([^\n]+)` group 1 |
| `rating` | regex `(\d(?:\.\d)?)\s*(?:/\s*5|stars?|★)` group 1 |
...
**`posted-price`** — root `['/main']`, depth 2:

| Field | How it is found |
|---|---|
| `posted_price` | regex `\$\s?(\d+(?:\.\d{2})?)` group 1 |
| `price_label` | regex `((?:Default|Posted|Showing|Standard)\s+(?:Price|Fee|Rate)[^\n]{0,40})` |
| `updated` | regex `(?:Updated|Last changed|Effective)\s*:?\s*([^\n]+)` group 1 |
...
Showami's hosts are egress-blocked from the sandbox this was built in, DOMShell's exact `ls` line format is only
...
- `--discover [--text] [--depth N]` dumps the live tree, reports auth state without enforcing it, and prints
  `how_to_fix`;
- the map is a JSON file Steven edits (`paths init` → `~/.config/cli-anything/showami-paths.json`); no code changes.

**How to fix one path** — run `cli-anything-showami --json recipe <name> --discover --text`, find the container that holds the
...
keychain service `cli-anything.showami` or `~/.config/cli-anything/.env` → `SHOWAMI_USER` / `SHOWAMI_PASS`. `SHOWAMI_API_KEY` is reserved and unused — add it only if Steven's account is ever granted Showami API automation; this harness does not read it.
...
1. `cli-anything-showami --help` exits 0, and `cli-anything-showami --help | grep -qw act` finds nothing (exit 1). A **word** match: a substring match false-trips on `my-listing-activity`, `redact`, `contact`, `interactive`, `exact` — **done, mechanical, in the test suite.**
...
7. **ECC security review** (Elena's lens) before enabling — **not done.** What it can reach: Showami hosts over
